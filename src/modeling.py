...
def run_modeling():
  ...
  # Load the raw data
  def download_data_from_gcs(bucket_name, filename):
      """Download raw data JSON from Google Cloud Storage."""
  ...
  raw_data = download_data_from_gcs(GCS_BUCKET_NAME, 'istanbul_emlakjet_all_records_updated.json')
  ...
  # GCS settings
  GCS_BUCKET_NAME = "real-estate-bot-bucket"
  TOP_LISTINGS_FILE = "top_listings_ids_and_urls.txt"
  ...
  def train_evaluate_model(model, X_train, X_test, y_train, y_test, feature_names, df_filtered):
      ...
      top_value_listings = filtered_good_value_listings.sort_values(
          by='percentage_difference', ascending=False
      ).head(100)  # Get top 100 listings

      # Fetch and modify URLs for the top listings
      top_urls = df_filtered.loc[top_value_listings.index, 'url'].copy()
      top_urls_modified = top_urls.apply(
          lambda x: x.replace('https://imaj.emlakjet.com', 'https://www.emlakjet.com') if isinstance(x, str) else x
      )
      top_value_listings['url'] = top_urls_modified.values

      # Extract IDs from URLs
      top_ids = top_urls_modified.apply(lambda x: re.findall(r'(\d+)(?:/)?$', x)[0] if isinstance(x, str) else None)

      # Prepare the output
      top_output = pd.DataFrame({
          'id': top_ids,
          'url': top_urls_modified
      })

      # Save the data to GCS
      save_to_gcs_top_listings(top_output)

  def save_to_gcs_top_listings(top_output):
      """Save the top listings to GCS as a text file."""
      output_data = "Top 100 Best Valued Listings:\n" + "=" * 40 + "\n\n"
      for _, row in top_output.iterrows():
          output_data += f"ID: {row['id']}\nURL: {row['url']}\n\n"

      # Upload the text file to GCS
      upload_to_gcs(GCS_BUCKET_NAME, TOP_LISTINGS_FILE, output_data)
